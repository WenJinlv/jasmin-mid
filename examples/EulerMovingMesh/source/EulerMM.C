...
#ifndef included_EulerMM_C
#define included_EulerMM_C
...
*************************************************************************
*/
EulerMM::EulerMM(const string& object_name,
             tbox::Pointer<tbox::Database> input_db,
             tbox::Pointer< appu::DeformingGridInputUtilities2 > grid_tool,
             tbox::Pointer<geom::DeformingGridGeometry<NDIM> > grid_geom)
{
#ifdef DEBUG_CHECK_ASSERTIONS
   ...
   // 计算一些参数.
   hier::BoxList<NDIM> boxes_list(grid_geom->getPhysicalDomain());
   hier::Box<NDIM> bbox(boxes_list.getBoundingBox());
   d_ncells = bbox.numberCells();
...
void EulerMM::registerModelVariables()
{
   // 变量数据库.
...
*************************************************************************
*/
void EulerMM::registerJaVisDataWriter(
...
*  初始化积分构件.
*********************************************************************/
void EulerMM::initializeComponent(algs::IntegratorComponent<NDIM>* intc) const
{
#ifdef DEBUG_CHECK_ASSERTIONS
   assert(intc!=NULL);
#endif
...
*************************************************************************
*/
void EulerMM::initializePatchData( hier::Patch<NDIM>& patch,
                                 const double  time,
...
*************************************************************************
*/
double EulerMM::getPatchDt(hier::Patch<NDIM>& patch,
                          const double  time,
                          const bool    initial_time,
...
*  实现数值构件.
*********************************************************************/
void EulerMM::computeOnPatch(hier::Patch<NDIM>& patch,
                          const double  time,
                          const double  dt,
                          const bool    initial_time,
                          const string& intc_name)
...
*  数值构件: 求Euler方程.
*********************************************************************/
void EulerMM::solveEulerEquation(hier::Patch<NDIM>& patch,
                                 const double  time,
                                 const double  dt,
                                 const bool    initial_time)
{
...
*  数值构件: 求梯度.
*********************************************************************/
void EulerMM::computeGradient(hier::Patch<NDIM>& patch,
                              const double  time,
                              const double  dt,
                              const bool    initial_time)
{
...
*  数值构件: 计算控制函数.
*********************************************************************/
void EulerMM::computeWCNEW(hier::Patch<NDIM>& patch,
                           const double  time,
                           const double  dt,
                           const bool    initial_time)
{
...
*  数值构件: 迭代求解控制函数.
*********************************************************************/
void EulerMM::wcnewIteration(hier::Patch<NDIM>& patch,
                             const double  time,
                             const double  dt,
                             const bool    initial_time)
{
...
*  数值构件: 移动网格.
*********************************************************************/
void EulerMM::moveMesh(hier::Patch<NDIM>& patch,
                       const double  time,
                       const double  dt,
                       const bool    initial_time)
{
...
*  数值构件: 移动网格之后, 修正数值解.
*********************************************************************/
void EulerMM::moveSolution(hier::Patch<NDIM>& patch,
                           const double  time,
                           const double  dt,
                           const bool    initial_time)
{
 ...
 ********************************************************************
 */ 
void EulerMM::setPhysicalBoundaryConditions(
...
//   tbox::pout << "patch_box=" << patch_box << endl;
...
/*
*************************************************************************
* 将EulerMM类对象的所有数据成员输出到指定的输出流.                        *
*************************************************************************
*/
void EulerMM::printClassData(ostream &os) const 
{

   os << "\nEulerMM::printClassData..." << endl;
   os << "EulerMM: this = " << (EulerMM*)this << endl;
   os << "d_object_name = " << d_object_name << endl;
   os << "d_grid_geometry = "
      << (geom::DeformingGridGeometry<NDIM>*)d_grid_geometry << endl;
...
/*
*************************************************************************
* 将EulerMM类的数据成员写入到重启动数据库.                                *
*************************************************************************
*/

void EulerMM::putToDatabase(tbox::Pointer<tbox::Database> db)
{
#ifdef DEBUG_CHECK_ASSERTIONS
   assert(!db.isNull());
#endif
