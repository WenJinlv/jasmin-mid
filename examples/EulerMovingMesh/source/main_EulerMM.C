//
// 文件名: main_Euler_moving_mesh.C
// 软件包: JASMIN application
// 版权  : (c) 2004-2010 北京应用物理与计算数学研究所
// 版本号: $Revision: 170 $
// 修改  : $Date: 2007-06-27 08:44:22 $
// 描述  : 主控程序(移动网格方法求解Euler方程组).
//
...
#include "DeformingGridGeometry.h"
...
/***************************************************************************
 * 基于JASMIN框架，采用移动网格r-自适应方法, 求解Euler方程组.
 *
 * 主控程序.
 *
 * 该应用实例调用如下的JASMIN框架类:                                    *
...
int main(int argc, char* argv[]) {
...

  {
    ...
    // 7.2 创建变形结构网格几何.
    tbox::Pointer<geom::DeformingGridGeometry<NDIM> > grid_geometry =
        new geom::DeformingGridGeometry<NDIM>("GridGeometry",
                                              coordinate_system);
    ...
    EulerMM* euler_model = new EulerMM(
        "EulerMM", input_db->getDatabase("EulerMM"), grid_tool, grid_geometry);

    // 7.5 创建网格层时间积分算法.
    EulerLevelIntegratorMM* Euler_level_integrator = new EulerLevelIntegratorMM(
        "EulerLevelIntegratorMM",
        input_db->getDatabase("EulerLevelIntegratorMM"), euler_model,
        grid_geometry,
    ...
    tbox::Pointer<appu::JaVisDataWriter<NDIM> > javis_data_writer =
        new appu::JaVisDataWriter<NDIM>("Euler JaVis Writer", viz_dump_dirname,
