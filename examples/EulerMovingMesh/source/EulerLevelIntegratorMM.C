...
#include "EulerLevelIntegratorMM.h" 
#include "EulerMM.h" 
...
* 构造函数.
*****************************************************************/
EulerLevelIntegratorMM::EulerLevelIntegratorMM(
...
*  创建并初始化所有积分构件.
*****************************************************************/
void EulerLevelIntegratorMM::initializeLevelIntegrator(
...
* 初始化网格层.
*****************************************************************/
void EulerLevelIntegratorMM::initializeLevelData(
...
* 计算时间步长.
*****************************************************************/
double EulerLevelIntegratorMM::getLevelDt(
...
* 积分一个时间步长.
*****************************************************************/
int EulerLevelIntegratorMM::advanceLevel(
                const tbox::Pointer< hier::BasePatchLevel<NDIM> > level, 
                ...
                double&      actual_dt)
{
    ...
    // 复制数据: current->new.
    d_cycle_copy_intc-> copyPatchData(patch_level,
                                      current_time);
...
* 接收数值解.
*****************************************************************/
void EulerLevelIntegratorMM::acceptTimeDependentSolution(
                const tbox::Pointer< hier::BasePatchLevel<NDIM> > level, 
...
* 从输入文件中读取参数值.
*****************************************************************/
void EulerLevelIntegratorMM::getFromInput(
...
* 从重启动数据库中读取参数值.
*****************************************************************/
void EulerLevelIntegratorMM::getFromRestart()
{
...
* 输出数据成员到重启动数据库. 
*****************************************************************/
void EulerLevelIntegratorMM::putToDatabase(
         tbox::Pointer<tbox::Database> db)
{
#ifdef DEBUG_CHECK_ASSERTIONS
    assert(!db.isNull());
