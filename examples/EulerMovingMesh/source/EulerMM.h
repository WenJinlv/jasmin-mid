...
#ifndef included_EulerMM
#define included_EulerMM
...
/**
 * @brief 类 EulerMM 在单个网格片, 提供数值计算子程序, 
 * 基于移动网格方法求解Euler方程组.
 * 
...
public:
   ...
   EulerMM(const string& object_name,
   ...
   /**
    * @brief 为指定名称的归约构件, 在单个网格片上计算迭代误差.
    *
    * @param error        输入输出参数, 指针, 指向归约向量.
    * @param len          输入参数, 整型, 归约向量的长度.
    * @param patch        输入参数, 网格片类, 待归约的网格片.
    * @param time         输入参数, 双精度浮点型, 表示当前时刻.
    * @param dt           输入参数, 双精度浮点型, 表示当前时间步长.
    * @param intc_name    输入参数, 字符串, 归约构件的名称.
    */
   void reduceOnPatch( 
                    double *error,
                    int     len,
