...
c variables in 2d cell indexed         
     ...
     &     velocity(CELL3dVECG(ifirst,ilast,gcw),0:NDIM-1),
     &     pressure(CELL3dVECG(ifirst,ilast,gcw)),
     &     df(CELL3dVECG(ifirst,ilast,gcw))
c
c***********************************************************************     
c
      ...
      logical detonation
c
      drr=sqrt(dx(0)*dx(0)+dx(1)*dx(1)+dx(2)*dx(2))
c
      do ic2=ifirst2,ilast2
        xc(2) = xlo(2)+dx(2)*(dble(ic2-ifirst2)+half)
        do ic1=ifirst1,ilast1
     ...
     &                    +(xc(2)-rp(2))*(xc(2)-rp(2)))
                ...
                detonation = abs(xc(2)-0.5d0*(lp(2)+rp(2))).le.1.1*dx(2)
             endif
             if(detonation) then
                density(ic0,ic1,ic2)=cjd
                pressure(ic0,ic1,ic2)=cjp
                df(ic0,ic1,ic2)=1.0d0
             else
                density(ic0,ic1,ic2)=djd
                pressure(ic0,ic1,ic2)=smallr
                df(ic0,ic1,ic2)=0.0d0
             endif
             do k=0,NDIM-1
                velocity(ic0,ic1,ic2,k)=0.0d0
             enddo
          enddo
        enddo
      enddo
