...
#include "FederationUniRectangularGridGeometry.h"
 ...
 *       联邦均匀矩形网格几何 geom::FederationUniRectangularGridGeometry<NDIM>,
...
int main(int argc, char* argv[]) {
...

  {
    ...
    // (2) 创建联邦均匀矩形网格几何.
    tbox::Pointer<geom::FederationUniRectangularGridGeometry<NDIM> >
        grid_geometry = new geom::FederationUniRectangularGridGeometry<NDIM>(
    ...
    for (int fd = 0; fd < nfederals; fd++) {
      linadv_level_integrator[fd] = new LinAdvLevelIntegrator(
          "LinAdvLevel_" + grid_geometry->getFederalGeometry(0)->getName(),
