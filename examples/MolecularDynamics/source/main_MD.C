...
int main(int argc, char* argv[]) {
...

  {
    ...
    MD* MD_model = new MD("MD", input_db->getDatabase("MD"), grid_geometry);

    // (5) 网格层显式时间积分算法类.
    MDLevelIntegrator* MD_level_integrator = new MDLevelIntegrator(
        "MDLevelIntegrator", input_db->getDatabase("MDLevelIntegrator"),
        MD_model, MD_model, grid_geometry);
    ...
    tbox::Pointer<appu::JaVisDataWriter<NDIM> > viz_data_writer =
        new appu::JaVisDataWriter<NDIM>("MD_JaVis_Writer", viz_dump_dirname,
                                        viz_number_procs_per_file, javis_db);

    // (8) 注册可视化输出器类对象到用户网格片时间积分算法类.
    MD_model->registerJaVisDataWriter(viz_data_writer);

    // 8. *** 初始化网格片层次结构和所有网格片上的数据. ***

    // (1) 创建初始的自适应结构网格, 为数据片调度内存空间,
    //     初始化数据片在初始时刻的值.
    if (!time_integrator->initializeHierarchy(true)) {
    ...
    /*
            tbox::Statistician *statistician =
       tbox::Statistician::getStatistician();
            statistician->finalize();
            if (tbox::MPI::getRank() == 0) {
              double aa6;
              aa6 = statistician->getGlobalPatchStatSum(
                      statistician->getPatchStatId("number_particles_on_patch"),
       0);
              tbox::plog<<" the number of processor:
       "<<tbox::MPI::getNodes()<<endl;
              tbox::plog<<" the number of particles: "<< aa6 <<endl;
              aa6 = aa6/1024/1024*(NDIM*2*8+4*(NDIM+2))/tbox::MPI::getNodes();
              tbox::plog<<" the memory of particles on single node: "<< aa6<<"
       M"<<endl;
            }
    */
    ...
    int integrator_step = time_integrator->getIntegratorStep();

    while ((loop_time < loop_time_end) && time_integrator->stepsRemaining()) {
      integrator_step = time_integrator->getIntegratorStep() + 1;

      tbox::pout << "\n\n++++++++++++++++++++++++++++++++++++++++++++" << endl;
      tbox::pout << "At begining of timestep # " << integrator_step - 1 << endl;
      tbox::pout << "Simulation time is " << loop_time << ", " << 6 % 3 << endl;
      tbox::pout << "++++++++++++++++++++++++++++++++++++++++++++\n\n" << endl;

      MD_model->setIntegratorStep(integrator_step);
      // 将数值解推进一个时间步, 返回其时间步长.
      ...
      tbox::pout << "\n\n++++++++++++++++++++++++++++++++++++++++++++" << endl;
      tbox::pout << "At end of timestep # " << integrator_step - 1 << endl;
      ...
      // 在指定时刻, 将相应数据写入重启动文件.
      if (write_restart) {
        if ((integrator_step % restart_interval) == 0) {
          tbox::RestartManager::getManager()->writeRestartFile(
              restart_write_dirname, integrator_step);
        }
      }
      // 在指定时刻, 输出可视化数据场.
      if (viz_dump_data) {
        if ((integrator_step % viz_dump_interval) == 0) {
          viz_data_writer->writePlotData(patch_hierarchy, integrator_step,
                                         loop_time);
        }
      }
    }
    ...
    // 模拟结束, 释放资源.
    if (MD_model) delete MD_model;
    if (MD_level_integrator) delete MD_level_integrator;
  }
