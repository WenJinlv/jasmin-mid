...
// 描述  :  Euler方程组：Lagrange方法在单个网格片上的数值计算子程序.
//

#ifndef included_MBSingDemo_C
#define included_MBSingDemo_C
...
*************************************************************************
*/
MBSingDemo::MBSingDemo(const string& object_name,
         ...
         tbox::Pointer<geom::MultiblockDeformingGridGeometry<NDIM> > grid_geom)
{
   ...
   // 创建数学运算.
   d_center_math_ops = new math::PatchCellDataBasicOps<NDIM,double>();
   d_node_math_ops   = new math::PatchNodeDataBasicOps<NDIM,double>();
...
MBSingDemo::~MBSingDemo() 
{ 

tbox::RestartManager::getManager()->unregisterRestartItem(d_object_name);

if(d_center_math_ops) delete d_center_math_ops;
if(d_node_math_ops)   delete d_node_math_ops;
...
void MBSingDemo::registerModelVariables()
{
   // 变量数据库.
...
*************************************************************************
*/
void MBSingDemo::registerJaVisDataWriter(
...
*  初始化积分构件.
*********************************************************************/
void MBSingDemo::initializeComponent(algs::IntegratorComponent<NDIM>* intc) const
{
#ifdef DEBUG_CHECK_ASSERTIONS
   ...
   if(intc_name=="INIT") {  // 属主网格层初值构件 : 为网格坐标和数值解赋初值.
        ...
        intc->registerPatchData(d_center_var_current_id);
        intc->registerPatchData(d_node_var_current_id);
   ...
   }else if(intc_name=="OUTERNODE_SUMMING") { // 外表面结点累加和构件: 求结点量的环绕累加和.
        intc->registerPatchData(d_node_var_current_id);
   }else if(intc_name=="ACCEPT_SOLUTION") { //复制构件: 接收数值解.
        intc->registerCopyPatchData(d_center_var_current_id,
                                    d_center_var_new_id);
   }else if(intc_name=="CURRENT_ALLOC_PATCH_DATA"){ // 内存构件: 为当前值数据片调度内存空间.
...
*************************************************************************
*/
void MBSingDemo::initializePatchData( hier::Patch<NDIM>& patch,
                                 const double  time,
...
*************************************************************************
*/
double MBSingDemo::getPatchDt(hier::Patch<NDIM>& patch,
                          const double  time,
                          const bool    initial_time,
...
*  实现数值构件.
*********************************************************************/
void MBSingDemo::computeOnPatch(hier::Patch<NDIM>& patch,
                          const double  time,
                          const double  dt,
                          const bool    initial_time,
                          const string& intc_name)
{
   ...
   #ifdef DEBUG_CHECK_ASSERTIONS
   TBOX_ASSERT(intc_name=="SUMMING" || intc_name=="PREP_SOLUTION" 
                                    || intc_name=="POST_SOLUTION");
   #endif
   ...
   if(intc_name=="SUMMING") {
   ...
   } else if(intc_name=="PREP_SOLUTION" || intc_name=="POST_SOLUTION") {
      ...
      double factor=1.0;
      if(intc_name=="PREP_SOLUTION" && clone_number>0 ) factor=clone_number+1.0;
      if(intc_name=="POST_SOLUTION" && number_clones>0) factor=2.0/(number_clones*(number_clones+1.0));

      d_center_math_ops->scale(center_var_current,
                        factor,
                        center_var_current,
                        center_var_current->getGhostBox());

      d_node_math_ops->scale(node_var_current,
                        factor,
                        node_var_current,
                        node_var_current->getGhostBox());
...
// 求密度的和.
void MBSingDemo::summing_density(hier::Patch<NDIM>& patch,
                                     const double  time,
                                     const double  dt,
                                     const bool    initial_time)
{
   ...
   for(int btype=2; btype<=NDIM; btype++) {
      ...
      for (int num=0; num < loc_indexes.size(); num++) {
         ...
         for(int p=0; p<nblocks; p++) {
             ...
             d_center_math_ops->add(center_var_scratch,
                             center_var_scratch,
                             patches[num][p]
 ...
 ********************************************************************
 */ 
void MBSingDemo::setPhysicalBoundaryConditions(
...
/*
*************************************************************************
* 将MBSingDemo类对象的所有数据成员输出到指定的输出流.                        *
*************************************************************************
*/
void MBSingDemo::printClassData(ostream &os) const 
{

   os << "\nMBSingDemo::printClassData..." << endl;
   os << "MBSingDemo: this = " << (MBSingDemo*)this << endl;
...
/*
*************************************************************************
* 将MBSingDemo类的数据成员写入到重启动数据库.                                *
*************************************************************************
*/

void MBSingDemo::putToDatabase(tbox::Pointer<tbox::Database> db)
{
}
...
*************************************************************************
*/
void MBSingDemo::getFromRestart()
{
}
