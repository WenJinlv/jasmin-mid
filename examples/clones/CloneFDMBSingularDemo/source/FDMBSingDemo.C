                          ...
                          const string& intc_name)
{
   ...
   if(intc_name=="SUMMING") {
      ...
      double factor=1.0;
      if(number_clones>0 ) factor=1.0/number_clones;

      d_math_ops->scale(center_var_current,
                        factor,
                        center_var_current,
                                     ...
                                     const bool    initial_time)
{
   ...
   tbox::plog<< "federal_" << pgeom->getFederalNumber()
   ...
   for(int btype=1; btype<=NDIM; btype++) {
      ...
      for (int num=0; num < fill_boxes.size(); num++) {
         ...
         tbox::plog << "\tfederal_regular_fill_box="<< fill_box
                    << ", type=" << btype
                    << ", loc=" << loc_indexes[num]
                    << endl;
   ...
   for(int btype=1; btype<=NDIM; btype++) {
      ...
      for (int num=0; num < fill_boxes.size(); num++) {
         ...
         tbox::plog << "\tblock_regular_fill_box="<< fill_box << ", type=" << btype
                    << ", loc=" << loc_indexes[num]
                    << endl;
   ...
   for(int btype=2; btype<=NDIM; btype++) {
      ...
      for (int num=0; num < loc_indexes.size(); num++) {
         ...
         tbox::plog << "\tblock_internal_fill_box="<< fill_boxes(num)
                    << ", type=" << btype
                    << ", loc=" << loc_indexes[num]
                    << endl;
   ...
   for(int btype=2; btype<=NDIM; btype++) {
      ...
      for (int num=0; num < loc_indexes.size(); num++) {
         ...
         tbox::plog << "\tblock_physical_fill_box="<< fill_boxes(num)
                    << ", type=" << btype
                    << ", loc=" << loc_indexes[num]
                  ...
                  const string& intc_name)
{
   ...
   #ifdef DEBUG_CHECK_ASSERTIONS
   TBOX_ASSERT(intc_name=="PREP_SOLUTION");
   #endif
