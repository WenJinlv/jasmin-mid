  ...
  d_grid_geometry(grid_geom)
{
   ...
   d_need_clone_param = true ;
   d_number_clones = ((hier::Federal *)grid_geom)->d_number_clones;
   // 创建所有变量及数据片索引号
...
void SnSweep::registerModelVariables()
{
   ...
   int mg_depth; 
   mg_depth = d_mg;
   if( d_number_clones > 1 ){
      int nb00 = d_mg / d_number_clones;
      int nb22 = d_mg % d_number_clones; 
      mg_depth = nb00;
      if(nb22 > 0) mg_depth = nb00 + 1;
   }
   d_loc2glob.resizeArray( mg_depth );
   d_mg_end2 = mg_depth - 1 ; 
   d_mg_depth = mg_depth;
   ...
   tbox::Pointer<hier::Variable<NDIM> > var_fi
              = new pdat::GroupNodeVariable<NDIM,double>("fi", mg_depth,d_ms); // GroupNode类型

   tbox::Pointer<hier::Variable<NDIM> > var_sgm_t
              = new pdat::GroupNodeVariable<NDIM,double>("sgm_t",mg_depth,1); // GroupNode类型
   ...
   tbox::Pointer<hier::Variable<NDIM> > var_qs
              = new pdat::GroupNodeVariable<NDIM,double>("qs", mg_depth, 1); // GroupNode类型

   tbox::Pointer<hier::Variable<NDIM> > var_qnw
              = new pdat::GroupNodeVariable<NDIM,double>("qnw", mg_depth, d_ms); // GroupNode类型
...
void SnSweep::initializeComponent(algs::IntegratorComponent<NDIM>* intc) const
{
   ...
   }else if(intc_name=="BCAST_SOLUTION") { // .
        intc->registerPatchData(d_wf_new_id);
   }else if(intc_name=="COLLECTION_SOLUTION") { //.
        intc->registerPatchData(d_wf_new_id);
 ...
 *  输出数据成员到重启动数据库.
 ************************************************************************/
void SnSweep::putToDatabase(tbox::Pointer<tbox::Database> db)
{
#ifdef DEBUG_CHECK_ASSERTIONS
   assert(!db.isNull());
#endif
   db->putInteger("d_clone_number", d_clone_number);
   db->putInteger("d_mg_begin", d_mg_begin);
   db->putInteger("d_mg_end", d_mg_end);
   db->putIntegerArray("d_loc2glob", d_loc2glob);
}
/*************************************************************************
 *  从重启动数据库读取数据. 
 ************************************************************************/
void SnSweep::getFromRestart()
{
   ...
   d_clone_number = db->getInteger("d_clone_number");
   d_mg_begin = db->getInteger("d_mg_begin");
   d_mg_end = db->getInteger("d_mg_end");
   d_loc2glob = db->getIntegerArray("d_loc2glob");
}
      ...
      const string& intc_name)
{
   ...
   assert(intc_name=="INIT");
      ...
      if(d_need_clone_param){
        d_need_clone_param = false;
        const int clone_number  = pgeom->getCloneNumber();
        d_clone_number = clone_number ;
        ...
        if( false ){
          ...
          int nb22 = d_mg%d_number_clones; 
          if(nb22>0){
            if( d_number_clones<nb22 ){
              d_mg_begin = nb00*d_clone_number + d_clone_number;
              d_mg_end = d_mg_begin + nb00;
//              d_mg_end2 = d_mg_end;
            }else{
              d_mg_begin = nb00*d_clone_number + d_clone_number;
              d_mg_end = d_mg_begin + nb00 -1 ;
//              d_mg_end2 = d_mg_end+1;
            } 
          } 
          if(nb22==0){
            d_mg_begin = nb00*d_clone_number;
            d_mg_end = d_mg_begin + nb00 -1 ;
//            d_mg_end2 = d_mg_end;
          }    
        }else { 
...
//          d_mg_end2 = d_mg_end;
      ...
      if( true ){
        if( d_number_clones > 1 ){
          int loc_g =0;
          d_mg_begin = 0;
          for(int b=0; b<d_mg; b++) {
            int my_rank = b % d_number_clones; 
            if(my_rank == d_clone_number){
              d_loc2glob[loc_g] = b;
              loc_g ++;
            }
          } 
   ...
   if (initial_time) {
      ...
      init_( BOX_INDEX(patch_box),
             ...
             d_sn_weight.getPointer(),
             ABC_DRESS,
             d_mg_begin, d_mg_end, d_mg_end2,
             d_loc2glob.getPointer()
             );
                             ...
                             const string& intc_name)
{
   if(intc_name=="SCATTER") {  // 计算散射源项.
       ...
       computeSourceItemOnPatch(patch,dt); 
 ...
 *  计算外源项
 ********************************************************************/
void SnSweep::computeSourceItemOnPatch(hier::Patch<NDIM>& patch, 
                                        const double dt)
{
...
//   tbox::Pointer< pdat::NodeData<NDIM,double> >   wf = patch.getPatchData(d_wf_new_id);
   ...
   tbox::Pointer< pdat::GroupNodeData<NDIM,double> > qw 
        = new pdat::GroupNodeData<NDIM,double>(patch_box,d_mg_depth,d_ms,0);
#ifndef TESTING
   qw->fillAll(0.e0);
