   ...
   d_grid_geometry(grid_geometry)
{
   ...
   snsweep_model->setSolutionCoeff(d_dt);
    ...
    t_compute_sweep = tbox::TimerManager::getManager()->
                            getTimer("Sn::SnSweepLevelIntegrator::compute_sweep");

   t_comm_bcast = tbox::TimerManager::getManager()->
                          getTimer("Sn::SnSweepLevelIntegrator::comm_bcast");
        ...
        tbox::Pointer<algs::IntegratorComponentManager<NDIM> > manager)
{
  ...
  d_bcast_solution = new algs::CloneBcastComponent<NDIM>(
                           "BCAST_SOLUTION", d_snsweep_model, manager);

  d_collection_solution = new algs::CloneReductionComponent<NDIM>(
                           "COLLECTION_SOLUTION", d_snsweep_model, manager);
                ...
                double&      actual_dt)
{
    ...
    // 计算外源项
    t_compute_sweep->start();
    d_sourcew_intc->computing(level,current_time,actual_dt);
    t_compute_sweep->stop();

    if(first_step) {
      tbox::MPI::barrier();
      t_comm_bcast->start();
      d_collection_solution->reduce(level, xfer::RefineUtilities::SUM);
    ...
    tbox::pout<<"#iter = "<<iter<<", error="<<wf_err_norm;

    // 设置新值数据片的时刻.
    d_new_context_intc->setTime(level, current_time+actual_dt);

#ifdef TESTING
//     tbox::plog<< "(7) SnSweepLevelIntegrator::allocate memory: ---- "<<endl;
