...
#include "CellData.h"
#include "SideData.h"
#include "FederalUniRectangularPatchGeometry.h"
...
void LinAdv::initializeComponent(algs::IntegratorComponent<NDIM>* intc) const
{
   ...
   if ( intc_name=="ALLOC_NEW_DATA") {  // 内存构件.
         ...
         intc->registerInitPatchData(d_uval_current_id);
          ...
          intc->registerPatchData(d_uval_scratch_id);
    } else if ( intc_name=="REDUCTION_SOLUTION") {
          intc->registerPatchData(d_uval_new_id);
    } else {
      ...
      const string& intc_name)
{
   ...
   if (initial_time) {
      ...
      const tbox::Pointer<geom::FederalUniRectangularPatchGeometry<NDIM> > 
                          ...
                          const string& intc_name)
{
   ...
   const tbox::Pointer<geom::FederalUniRectangularPatchGeometry<NDIM> > pgeom
                          ...
                          const string& intc_name)
{
   ...
   assert(   intc_name=="COMPUTE_FLUX"  || intc_name=="CONSER_DIFF" 
          || intc_name=="POST_SOLUTION" || intc_name=="PREP_SOLUTION");
#endif
   ...
   if(intc_name=="COMPUTE_FLUX" ) {  
     ...
     const int number_clones = patch.getPatchGeometry()->getNumberClones();
     double ratio=1.0;
     if(number_clones>0) ratio = 2.0/(number_clones*(number_clones+1.0));
     d_center_math_ops->scale(uval_current,
                              ratio,
                              uval_current,
                              uval_current->getGhostBox());
...
//         tbox::plog << endl << "federal=" << pgeom->getFederalNumber()
//                    << "clones=" << pgeom->getCloneNumber() << endl; 
//         tbox::plog << endl << "current=" << endl;
//         uval_current->print(uval_current->getGhostBox(),tbox::plog);
//         tbox::plog << endl << "scratch=" << endl;
//         uval_scratch->print(uval_scratch->getGhostBox(),tbox::plog);

     const int clone_number  = pgeom->getCloneNumber();
     double ratio = clone_number+1.0;
     if(clone_number<0) ratio=1.0;
     d_center_math_ops->scale(uval_current,
                              ratio,
                              uval_current,
                              uval_current->getGhostBox());
     d_center_math_ops->scale(uval_scratch,
                              ratio,
                              uval_scratch,
                              uval_scratch->getGhostBox());
...
{
#ifdef DEBUG_CHECK_ASSERTIONS
   assert(intc_name == "INIT_SET_VALUE" || intc_name == "PREP_SOLUTION"); 
#endif
