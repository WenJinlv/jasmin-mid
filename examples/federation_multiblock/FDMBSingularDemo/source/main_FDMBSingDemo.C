 ...
 *************************************************************************
 *
 * @brief 验证JASMIN框架中联邦单层多块变形结构网格功能的完整性和正确性.
...
int main(int argc, char* argv[]) {
...

  {
    ...
    tbox::Pointer<appu::DeformingGridConstructionUtilities<NDIM, double> >
        grid_tool1;
    tbox::Pointer<appu::DeformingGridInputUtilities2> grid_tool2;
    if (input_db->keyExists("DeformingGridConstructionUtilities")) {
      if (!is_from_restart) {
        grid_tool1 = new appu::DeformingGridConstructionUtilities<NDIM, double>(
            "DeformingGridConstructionUtilities",
            input_db->getDatabase("DeformingGridConstructionUtilities"),
            grid_geometry);
      }
    } else if (input_db->keyExists("DeformingGridInputUtilities2")) {
      grid_tool2 = new appu::DeformingGridInputUtilities2(
    ...
    if (input_db->keyExists("DeformingGridConstructionUtilities")) {
      for (int fd = 0; fd < nfederals; fd++) {
        singdemo_patch_model[fd] = new FDMBSingDemo(
            ...
            input_db->getDatabase("FDMBSingDemo"), grid_tool1,
      ...
      for (int fd = 0; fd < nfederals; fd++) {
        singdemo_patch_model[fd] = new FDMBSingDemo(
            grid_geometry->getFederalGeometry(fd)->getName() + "_model",
            input_db->getDatabase("FDMBSingDemo"), grid_tool2,
            grid_geometry->getFederalGeometry(fd));
      }
    }
