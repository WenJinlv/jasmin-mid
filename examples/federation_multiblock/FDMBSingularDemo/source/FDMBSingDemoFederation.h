...
#ifndef included_FDMBSingDemoFederation
#define included_FDMBSingDemoFederation
...
public:
    ...
    *  -# 定义变量和数据片.
    */
   FDMBSingDemoFederation(const string& object_name,
          tbox::Pointer< hier::FederationMultiblockGridGeometry<NDIM> > grid_geometry);

   /*!
    * @brief 析构函数.
    */
   virtual ~FDMBSingDemoFederation();
...
private:
   ...
   /*!@brief 上下文: 当前值, 新值, 演算值 */
   tbox::Pointer< hier::VariableContext > d_current;
   tbox::Pointer< hier::VariableContext > d_scratch;

   /*!@brief 数据片索引号. */
   int d_coords_current_id, d_coords_scratch_id;
   int d_center_var_current_id, d_center_var_scratch_id;
