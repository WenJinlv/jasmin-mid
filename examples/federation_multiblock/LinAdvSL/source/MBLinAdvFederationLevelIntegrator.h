 //
// 文件名:	MBLinAdvFederationLevelIntegrator.h
...
#ifndef included_MBLinAdvFederationLevelIntegrator
#define included_MBLinAdvFederationLevelIntegrator
 ...
 * 实现线性对流方程在网格层上的求解流程.
 */
class MBLinAdvFederationLevelIntegrator 
...
public:
    ...
    */     
   MBLinAdvFederationLevelIntegrator(
    ...
    * @brief 析构函数.
    */
   virtual ~MBLinAdvFederationLevelIntegrator();
    ...
    * MBLinAdv::initializePatchData(), 逐个网格片地完成初始时刻的数据初始化.
    */
    ...
    * MBLinAdv::getPatchDt(), 逐个网格片地计算稳定时间步长.
    */
   double getLevelDt(
    ...
    * 相关的网格片计算函数见 MBLinAdv::computeOnPatch().
    */ 
...
private:
   ...
   /*!@brief 网格层时间积分算法. */
   MBLinAdvLevelIntegrator * d_level_integrator[MAX_FEDERALS];

   /*!@brief 网格片时间积分算法. */
   MBLinAdvFederation* d_patch_strategy;
