...
// 描述  : 主控程序(联邦多块均匀矩形结构网格上, 求解线性对流对流问题).
//

#include "CartesianCoordinates.h"
#include "FederationMultiblockUniRectangularGridGeometry.h"
 ...
 *************************************************************************
 *
 * @brief 基于JASMIN框架的联邦单层多块均匀矩形结构网格, 求解线性对流问题.
 ...
 *       联邦多块均匀矩形网格几何
 *       geom::FederationMultiblockUniRectangularGridGeometry<NDIM>,
 *       联邦网格片层次结构(多块) hier::FederationMultiblockPatchHierarchy<NDIM>
 *    -# 网格片积分算法 MBLinAdvFederation, MBLinAdv
 *       - 应用级: 提供求解线性对流方程的数值计算子程序
 *    -# 网格层积分算法 MBLinAdvFederationLevelIntegrator,
 *       MBLinAdvLevelIntegrator
...
int main(int argc, char* argv[]) {
...

  {
    ...
    // (2) 创建联邦均匀矩形网格几何.
    tbox::Pointer<geom::FederationMultiblockUniRectangularGridGeometry<NDIM> >
        grid_geometry =
            new geom::FederationMultiblockUniRectangularGridGeometry<NDIM>(
    ...
    MBLinAdv* linadv_advection_model[nfederals];
    for (int fd = 0; fd < nfederals; fd++) {
      linadv_advection_model[fd] = new MBLinAdv(
          grid_geometry->getFederalGeometry(fd)->getName() + "_model",
          input_db->getDatabase("MBLinAdv"));
    }
    MBLinAdvFederation* linadv_advection_federation_model =
        new MBLinAdvFederation("MBLinAdvFederation", grid_geometry);

    // (5) 创建网格层时间积分算法类.
    MBLinAdvLevelIntegrator* linadv_level_integrator[nfederals];
    for (int fd = 0; fd < nfederals; fd++) {
      linadv_level_integrator[fd] = new MBLinAdvLevelIntegrator(
          "MBLinAdvLevel_" + grid_geometry->getFederalGeometry(0)->getName(),
          linadv_advection_model[fd]);
    }
    MBLinAdvFederationLevelIntegrator* linadv_level_federation_integrator =
        new MBLinAdvFederationLevelIntegrator(
            "MBLinAdvLevelIntegrator", grid_geometry->getNumberFederals(),
    ...
    if (javis_dump_interval > 0) {
      federal_javis_writer = new appu::FederationJaVisDataWriter<NDIM>(
    ...
    if (javis_dump_interval > 0) {
      for (int fd = 0; fd < patch_hierarchy->getNumberFederals(); fd += 1) {
        // tbox::Pointer< appu::JaVisDataWriter<NDIM> >
        // javis_data_writer=federal_javis_writer->getDataWriter(fd);
        string writer_name =
            patch_hierarchy->getHierarchy(fd)->getGridGeometry()->getName();
        tbox::Pointer<appu::JaVisDataWriter<NDIM> > javis_data_writer =
            federal_javis_writer->getDataWriter(writer_name);
