...
* 构造函数.
*****************************************************************/
RotAdvLevelIntegrator::RotAdvLevelIntegrator(
...
/****************************************************************
* 根据刚性旋转问题, Goundov离散格式, 创建并初始化所有积分构件.
*****************************************************************/
void RotAdvLevelIntegrator::initializeLevelIntegrator(
...
* 初始化网格层数据.
*****************************************************************/
void RotAdvLevelIntegrator::initializeLevelData(
...
* 计算时间步长.
*****************************************************************/
double RotAdvLevelIntegrator::getLevelDt(
...
* 积分一个时间步长.
*****************************************************************/
int RotAdvLevelIntegrator::advanceLevel(
                const tbox::Pointer< hier::BasePatchLevel<NDIM> > level, 
...
*****************************************************************/
void RotAdvLevelIntegrator::synchronizeAdvanceLevel(
...
* 时间步序列中, 积分一个时间步长.
*****************************************************************/
void RotAdvLevelIntegrator::standardAdvanceLevel(
                const tbox::Pointer< hier::BasePatchLevel<NDIM> > level, 
                const double current_time,
                const double actual_dt,
                const bool   first_step)
...
* 接收数值解.
*****************************************************************/
void RotAdvLevelIntegrator::acceptTimeDependentSolution(
                const tbox::Pointer< hier::BasePatchLevel<NDIM> > level, 
...
* 接收数值解.
*****************************************************************/
void  RotAdvLevelIntegrator::synchronizeNewHierarchy(
                   const tbox::Pointer< hier::BasePatchHierarchy<NDIM> > hierarchy, 
...
* 接收数值解.
*****************************************************************/
bool RotAdvLevelIntegrator::usingRefinedTimestepping()
{
...
* 接收数值解.
*****************************************************************/
void RotAdvLevelIntegrator::synchronizeCoarserLevel(
      const tbox::Pointer< hier::BasePatchHierarchy<NDIM> > hierarchy, 
      ...
      const double coarser_old_time)
{
   ...
   // 校正细网格层覆盖的计算区域.
   d_sync_overlay_intc->synchronizeCoarserLevel(
                           hierarchy -> getPatchLevel(finer_level_number),
                           sync_time);
...
* 接收数值解.
*****************************************************************/
void RotAdvLevelIntegrator::tagCellsForRefinement(
               const tbox::Pointer< hier::BasePatchHierarchy<NDIM> > hierarchy,
               const int level_number,
...
* 输出数据成员到重启动数据库. 
*****************************************************************/
void RotAdvLevelIntegrator::putToDatabase(
...
* 从输入文件中读取参数值.
*****************************************************************/
void RotAdvLevelIntegrator::getFromInput(
    tbox::Pointer<tbox::Database> db,
...
* 从重启动数据库中读取参数值.
*****************************************************************/
void RotAdvLevelIntegrator::getFromRestart()
{
