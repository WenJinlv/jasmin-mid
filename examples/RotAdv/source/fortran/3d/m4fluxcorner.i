...
c     write(6,*) "         dt= ",dt
      do ic$3=ifirst$3-1,ilast$3+1
         do ic$2=ifirst$2,ilast$2
           do ic$1=ifirst$1,ilast$1+1
              if($2.eq.0.and.$3.eq.2.or.
     &           $2.eq.1.and.$3.eq.0.or.
     &           $2.eq.2.and.$3.eq.1) then
              advecspeed= (xlo($3)+dx($3)*(ic$3-ifirst$3+0.5))*omega($2)
     &                   -(xlo($2)+dx($2)*(ic$2-ifirst$2+0.5))*omega($3)
              else
     ...
     &                   -(xlo($3)+dx($3)*(ic$3-ifirst$3+0.5))*omega($2)
             ...
             flux$1(ic$1,$4)= dt*riemst*advecspeed
           enddo
         enddo
      enddo
')dnl
