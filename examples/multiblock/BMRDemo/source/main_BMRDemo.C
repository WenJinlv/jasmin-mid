 ...
 *           BMRDemo                   - 网格片时间积分数值计算子程序, *
 *                                              实现各类积分构件.            *
 *
 * 该例子演示方法：
 * 创建1个中心量和1个结点量，深度为1. 令中心量的初值等于1, 结点量的初值等于0.
 * 执行以下操作:
 *     (1) 中心量的值等于环绕该单元的所有单元中心量(含自身)的值的累加和.
 *     (2) 结点量的值等于环绕该结点的所有单元中心量的值的累加和.
 * 迭代两个时间步，可视化输出中心量和结点量.
 *
 ****************************************************************************
 */
...
int main(int argc, char* argv[]) {
...

  {
    ...
    tbox::Pointer<hier::AdvancedMultiblockGridGeometry<NDIM> >
        advanced_grid_geometry;

    tbox::Pointer<appu::DeformingGridInputUtilities2> grid_tool =
        new appu::DeformingGridInputUtilities2();

    advanced_grid_geometry = grid_tool->makeAdvancedMultiblockGridGeometry(
        "AdvancedMultiblockGridGeometry", coordinate_system,
    ...
    BMRDemo* euler_model =
        new BMRDemo("MBSingDemo", input_db->getDatabase("BMRDemo"), grid_tool,
                    grid_geometry);

    // 7.5 创建网格层时间积分算法.
    BMRDemoLevelIntegrator* Euler_level_integrator = new BMRDemoLevelIntegrator(
        "BMRDemoLevelIntegrator",
        input_db->getDatabase("BMRDemoLevelIntegrator"), euler_model,
        grid_geometry,
