...
// 描述  :	验证多块结构网格BMR模块：单网格片数值计算子程序.
//
 
#ifndef included_BMRDemo
#define included_BMRDemo
...
/**
 * @brief 类 BMRDemo 在单个网格片, 提供数值计算子程序, 
 * 验证BMR多块结构网格功能的正确性.
 * 
 * 实现网格片时间积分算法基类 algs::StandardComponentPatchStrategy.
 * 在单个网格片上, 验证几何奇异网格片的正确性.
 *
 * 实现如下纯虚函数:
...
public:
   ...
   BMRDemo(const string& object_name,
   ...
   /**
    * @brief 支撑指定名称的初值构件, 在单个网格片上执行初始化操作.
    *
    * @param patch_dst 输入参数, 网格片类, 表示需插值的目的网格片.
    * @param patch_src 输入参数, 网格片类, 表示插值的源网格片.
    * @param ratio_dst_to_src 输入参数, 整型向量, 表示目的网格片相对于源网格片的细化率.
    * @param time  输入参数, 双精度浮点型, 表示初始化的时刻.
    * @param initial_time 输入参数, 逻辑型, 真值表示当前时刻为计算的初始时刻.
    * @param intc_name 输入参数, 字符串, 表示初值构件的名称.
    *
    */
   virtual void BMRInterpolationPatchData(
      hier::Patch<NDIM>& patch_dst,
...
private:
   ...
   // 计算温度.
   void advancing_temperature(hier::Patch<NDIM>& patch,
                   const double  time,
                   const double  dt,
                   const bool    initial_time);
   ...
   tbox::Pointer< pdat::CellVariable<NDIM,double> > d_source;
   ...
   // BMR 插值量.
   int d_coords_bmr_id, d_center_var_bmr_id, d_node_var_bmr_id;         // < d_node_var, d_current > : 存储结点量.

   // 数据片的两种影像区宽度.
   hier::IntVector<NDIM> d_zeroghosts, d_oneghosts, d_twoghosts;

   // 数学运算.
   math::PatchCellDataBasicOps<NDIM,double>* d_math_ops;
