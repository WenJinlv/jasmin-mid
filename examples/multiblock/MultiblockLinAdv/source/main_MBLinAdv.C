 ...
 *************************************************************************
 *
 * @brief 基于JASMIN框架的多块拼接功能, 在单层多块拼接的均匀矩形结构网格上,
 * 求解线性对流方程.
 *
 * 该函数分以下几个步骤:
 ...
 *       多块均匀矩形网格几何 geom::MultiblockUniRectangularGridGeometry<NDIM>,
 *       多块网格片层次结构   hier::PatchHierarchy<NDIM>
 *    -# 网格片积分算法       MBLinAdv
 *       - 应用级: 提供求解线性对流方程的数值计算子程序
 *    -# 网格层积分算法       MBLinAdvLevelIntegrator
...
int main(int argc, char* argv[]) {
...

  {
    ...
    // (2) 创建均匀矩形网格几何.
    tbox::Pointer<geom::MultiblockUniRectangularGridGeometry<NDIM> >
        grid_geometry = new geom::MultiblockUniRectangularGridGeometry<NDIM>(
            "MultiblockCartesianGeometry",
            input_db->getDatabase("MultiblockCartesianGeometry"), coords);
    ...
    // (4) 创建网格片时间积分算法类（应用级:
    // 提供求解线性对流方程的数值计算子程序）.
    MBLinAdv* linadv_advection_model = new MBLinAdv(
        "MBLinAdv", input_db->getDatabase("MBLinAdv"), grid_geometry);

    // (5) 创建网格层时间积分算法类（应用级:
    // 提供基于网格层的线性对流方程求解流程）.
    MBLinAdvLevelIntegrator* linadv_level_integrator =
        new MBLinAdvLevelIntegrator("MBLinAdvLevelIntegrator",
                                    linadv_advection_model);
    ...
    if (javis_dump_interval > 0) {
      javis_data_writer = new appu::JaVisDataWriter<NDIM>(
          "MBLinAdv_JaVis_Writer", javis_dump_dirname,
